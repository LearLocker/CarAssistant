...
#### START - FUNCTION TO READ AN INPUT IMAGE ###################################
    ...
    def processImage(self, inpImage):
        ...
        return self.image, hls_result, gray, thresh, blur, canny
    ...
    def on_enter(self):  # Будет вызвана в момент открытия экрана
        ...
        while True:
            ...
            meanPts, result = self.draw_lane_lines(frame, thresh, minverse, draw_info)
            deviation, directionDev = self.offCenter(meanPts, frame)
            # Adding text to our final image
            finalImg = self.addText(result, curveRad, curveDir, deviation, directionDev)
